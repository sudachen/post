...
func cScryptPositions(providerId uint, id, salt []byte, startPosition, endPosition uint64, hashLenBits, options, n, r, p uint32) ([]byte, int, int) {
	mtx.Lock()
	defer mtx.Unlock()
	...
	cHashLenBits := C.uint(hashLenBits)
